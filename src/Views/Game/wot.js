...
import { getNameWot } from "../../Redux/Actions/Wot";
import Swal from "sweetalert2";
import { Link } from "react-router-dom";

class Wot extends Component {
  state = {
    country: "",
    nameCon: "",
    names: [],
    ...
    if (event.key === "Enter") {
      if (this.state.tmpName.length === 0) {
        ...
        await this.props.dispatch(
          getNameWot(this.state.tmpName, this.state.country)
        );
  ...
  render() {
    ...
    const games = this.props.game.filter(
      data => data.name !== "World of Tanks"
    );
    console.log(games);

    return (
      <Fragment>
        <div className="img-wot">
          <Container>
            <div className="pt-4">
...
export default connect(mapStateToProps)(Wot);
